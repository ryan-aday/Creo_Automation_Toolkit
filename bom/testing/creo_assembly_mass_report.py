...
def write_bom_sheet(
    ...
    for row in rows:
        ...
        if row.warnings:
            ws.cell(r, 22).fill = PatternFill("solid", fgColor="FFF2CC")
            ws.cell(r, 22).font = Font(color="9C6500", bold=True)

        if md.thumbnail_path:
            path_cell = ws.cell(r, 21)
...
def save_json_report(
    ...
    """Write a machine-readable sidecar used by downstream harness analysis."""
    json_path = output.with_suffix(".json")

    def row_dict(row: GroupRow) -> dict[str, Any]:
        md = row.metadata
        return {
            "indices": list(row.indices),
            "name": row.model,
            "type": row.model_type,
            "immediate_assembly": row.immediate_assembly,
            "immediate_assembly_index": row.immediate_assembly_index,
            "assembly_path": row.assembly_path,
            "depth": row.depth,
            "mass_unit": report_unit,
            "mass": format_mass(row.mass_kg_each, report_unit),
            "qty": row.qty,
            "total_mass": format_mass(row.total_mass_kg, report_unit),
            "qty_in_top_assembly": row.top_qty,
            "native_mass_unit": md.native_mass_unit,
            "material": md.material,
            "category": row.category,
            "owner": md.owner,
            "created_by": md.created_by,
            "last_modified_by": md.modified_by,
    ...
    payload = {
        "schema": "creo-assembly-mass-report/v3",
        "top_assembly": root.model,
        "report_mass_unit": report_unit,
        "derived_top_mass": format_mass(root.mass_kg, report_unit),
        "hierarchy": [row_dict(r) for r in hierarchy_rows],
        "unique_rollup": [row_dict(r) for r in unique_rows],
...
def run(settings: Settings) -> Path:
    ...
    json_output = save_json_report(
        settings.output, root, hierarchy_rows, unique_rows, report_unit, warnings
    )
    console_info(f"JSON sidecar: {json_output}")
